...
DEFINE_TEST(test_adjacent_find)
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        result = dpl::adjacent_find(make_new_policy<new_kernel_name<Policy, 0>>(exec), first, last, comp);
        ...
        result = dpl::adjacent_find(make_new_policy<new_kernel_name<Policy, 0>>(exec), first, last);
        wait_and_throw(exec);
...
DEFINE_TEST(test_find_first_of)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        {
            auto res =
                dpl::find_first_of(make_new_policy<new_kernel_name<Policy, 1>>(exec), first1, last1, first2, first2);
            wait_and_throw(exec);
