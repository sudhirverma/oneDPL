...
DEFINE_TEST(test_transform_inclusive_scan)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res2 = dpl::transform_inclusive_scan(make_new_policy<new_kernel_name<Policy, 1>>(exec), first1, last1,
...
DEFINE_TEST(test_transform_exclusive_scan)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = dpl::transform_exclusive_scan(make_new_policy<new_kernel_name<Policy, 2>>(exec), first1, last1,
                                                  first2, T1{}, ::std::plus<T1>(), [](T1 x) { return x * 2; });
        wait_and_throw(exec);
...
DEFINE_TEST(test_copy_if)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 last2, Size n)
    {
        ...
        auto res1 = dpl::copy_if(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1, first2,
                                 [](T1 x) { return x > -1; });
        wait_and_throw(exec);
