...
DEFINE_TEST(test_uninitialized_move)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 /* last2 */, Size n)
    {
        ...
        dpl::uninitialized_move(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1, first2);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        host_vals.retrieve_data();
...
DEFINE_TEST(test_uninitialized_move_n)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 /* last1 */, Iterator2 first2, Iterator2 /* last2 */, Size n)
    {
        ...
        dpl::uninitialized_move_n(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, n, first2);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        host_vals.retrieve_data();
...
DEFINE_TEST(test_replace_copy)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 /* last2 */, Size n)
    {
        ...
        dpl::replace_copy(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1, first2, value, T1(value + 1));
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        host_vals.retrieve_data();
...
DEFINE_TEST(test_replace_copy_if)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator2 /* last2 */, Size n)
    {
        ...
        dpl::replace_copy_if(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1, first2,
                             oneapi::dpl::__internal::__equal_value<T1>(value), T1(value + 1));
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
...
DEFINE_TEST(test_remove)
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        auto res1 = dpl::remove(make_new_policy<new_kernel_name<Policy, 0>>(exec), first, last, T1(222 + pos));
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
...
DEFINE_TEST(test_partition)
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        res = dpl::stable_partition(make_new_policy<new_kernel_name<Policy, 1>>(exec), first, last, unary_op);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
...
DEFINE_TEST(test_inplace_merge)
{
    ...
    operator()(Policy&& exec, Iterator first, Iterator last, Size n)
    {
        ...
        auto middle = dpl::stable_partition(host_keys.get(), host_keys.get() + n, [](const T& x) { return x % 2; });
        host_keys.update_data();

        dpl::inplace_merge(make_new_policy<new_kernel_name<Policy, 0>>(exec), first, first + (middle - host_keys.get()), last);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
...
DEFINE_TEST(test_stable_sort)
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Size n)
    {
        ...
        dpl::stable_sort(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        {
...
#           endif
        ...
        dpl::stable_sort(make_new_policy<new_kernel_name<Policy, 1>>(exec), first1, last1, ::std::greater<T1>());
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        host_keys.retrieve_data();
...
DEFINE_TEST(test_rotate_copy)
{
    ...
    operator()(Policy&& exec, Iterator1 first, Iterator1 last, Iterator1 result_first, Iterator1 /* result_last */, Size n)
    {
        ...
        dpl::rotate_copy(make_new_policy<new_kernel_name<Policy, 0>>(exec), first, first + 1, last, result_first);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        host_vals.retrieve_data();
...
DEFINE_TEST(test_set_difference)
{
               ...
               Iterator3 last3, Size n)
    {
        ...
        last3 = dpl::set_difference(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1, first2, last2, first3);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        int res_expect[na];
        host_res.retrieve_data();
        auto nres_expect = dpl::set_difference(host_keys.get(), host_keys.get() + na, host_vals.get(), host_vals.get() + nb, res_expect) - res_expect;
...
DEFINE_TEST(test_set_symmetric_difference)
{
               ...
               Iterator3 last3, Size n)
    {
        ...
        last3 = dpl::set_symmetric_difference(make_new_policy<new_kernel_name<Policy, 0>>(exec), first1, last1,
                                                first2, last2, first3);
#if _PSTL_SYCL_TEST_USM
        exec.queue().wait_and_throw();
#endif
        int res_expect[na + nb];
        retrieve_data(host_keys, host_vals, host_res);
        auto nres_expect = dpl::set_symmetric_difference(host_keys.get(), host_keys.get() + na, host_vals.get(),
