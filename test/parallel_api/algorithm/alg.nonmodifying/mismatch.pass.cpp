...
struct test_mismatch
{
    ...
    operator()(Policy&& exec, Iterator1 first1, Iterator1 last1, Iterator2 first2)
    {
        ...
        {
            const auto expected = dpl::mismatch(first1, last1, first2, ::std::equal_to<T>());
...
test_mismatch_by_type()
{
    ...
    for (size_t size = 0; size <= 100000; size = size <= 16 ? size + 1 : size_t(3.1415 * size))
    {
        ...
        }
        {
            ...
            //  We can't call dpl::mismatch with semantic below when size of second sequence less than size of first sequence
            if (in2.size() <= in.size())
            {
